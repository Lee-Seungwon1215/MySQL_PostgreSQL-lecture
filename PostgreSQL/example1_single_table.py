...
def setup_database(conn, table_name):
    with conn.cursor() as cur:
        ...
        cur.execute(f"CREATE INDEX idx_{table_name}_algorithm ON {table_name}(signature_algorithm)")
        cur.execute(f"CREATE INDEX idx_{table_name}_doc_hash ON {table_name}(document_hash)")
    conn.commit()
...
def main():
    ...
    try:
        ...
        row_count, table_bytes, index_bytes, total_bytes, toast_bytes = table_stats(
        ...
        for (
            ...
            print(
                f"table_size={mb(table_bytes):.2f} MB, "
                f"index_size={mb(index_bytes):.2f} MB, "
                f"toast_size={mb(toast_bytes):.2f} MB, "
